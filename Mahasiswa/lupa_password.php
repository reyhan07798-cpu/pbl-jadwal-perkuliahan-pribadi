...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['verify'])) {
    if (empty(trim($_POST["username"]))) $username_err = "Nama tidak boleh kosong."; else $username = trim($_POST["username"]);
    ...
    if (empty($username_err) && empty($email_err)) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                else { $username_err = "Nama atau Email tidak cocok dengan akun manapun."; }
            }
            $stmt->close();
        }
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['reset'])) {
    if (isset($_SESSION['verified_for_reset'])) {
        if (empty(trim($_POST["new_password"]))) $password_err = "Kata sandi baru tidak boleh kosong.";
        elseif (strlen(trim($_POST["new_password"])) < 6) $password_err = "Kata sandi harus memiliki minimal 6 karakter.";
        ...
        if (empty($password_err)) {
            ...
            if ($stmt = $conn->prepare($sql)) {
                ...
                if ($stmt->execute()) {
                    ...
                    $success_msg = "Kata sandi berhasil diubah! Anda akan dialihkan ke halaman login dalam 3 detik.";
                ...
                <div class="card shadow-sm">
                    <div class="card-body p-4">
                        ...
                        <?php if (!isset($_SESSION['verified_for_reset'])): ?>
                            <p class="text-center text-muted mb-4">Masukkan Nama dan email Anda untuk memverifikasi akun.</p>
                            ...
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                <div class="mb-3"><label for="username" class="form-label">Nama</label><input type="text" name="username" class="form-control <?php echo (!empty($username_err)) ? 'is-invalid' : ''; ?>" value="<?php echo htmlspecialchars($username); ?>"><div class="invalid-feedback"><?php echo $username_err; ?></div></div>
                        ...
                        <?php else: ?>
                            <p class="text-center text-muted mb-4">Akun terverifikasi. Silakan masukkan kata sandi baru Anda.</p>
                            <?php if(!empty($password_err)){ echo '<div class="alert alert-danger" role="alert">' . $password_err . '</div>'; } ?>
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                <div class="mb-3"><label for="new_password" class="form-label">Kata Sandi Baru</label><input type="password" name="new_password" class="form-control <?php echo (!empty($password_err)) ? 'is-invalid' : ''; ?>"><div class="invalid-feedback"><?php echo $password_err; ?></div></div>
                                <div class="d-grid"><button type="submit" name="reset" class="btn btn-primary">Ubah Kata Sandi</button></div>
                            </form>
                        <?php endif; ?>
                        <hr><p class="text-center mb-0">Sudah ingat akun Anda? <a href="login_mahasiswa.php">Masuk di sini</a>.</p>
                    </div>
                </div>
            </div>
        </div>
